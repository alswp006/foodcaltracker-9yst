 ...
 *   FoodCalTracker -> the app's display name
 *   MyFitnessPal 사용자들이 한국 음식 데이터 부정확함에 지쳐있는 지금, 사진 한 장으로 한식 칼로리를 즉시 인식하고 식단 목표까지 연결해주는 한국 특화 영양 트래커    -> the one-line description
 */
...
export default function Home() {
  ...
  return (
    <ScreenScaffold
      top={<Top title={<Top.TitleParagraph>FoodCalTracker</Top.TitleParagraph>} />}
    >
      {/* 시각 앵커: 헤드라인 + 카드 내 진입 버튼(부유 금지, display="block" 전체폭).
          데이터 앱이면 value를 <Amount typography="t1" />(핵심 숫자)로 교체하라. */}
      <SummaryHero
        label="FoodCalTracker"
        value={<Paragraph.Text typography="t2">MyFitnessPal 사용자들이 한국 음식 데이터 부정확함에 지쳐있는 지금, 사진 한 장으로 한식 칼로리를 즉시 인식하고 식단 목표까지 연결해주는 한국 특화 영양 트래커</Paragraph.Text>}
